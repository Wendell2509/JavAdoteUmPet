package br.com.generations.javadoteumpet;
...
class Main {
	...
	public static void main(String[] args) throws NumberFormatException {
		...
		do {
			...
			if (!nome.matches("^[A-zÀ-ú '´]+$")) { // .matches() verifica se o nome passado é compatível com os
													// caracteres de A a Z e com acentuação, caso contrário, retorna que
													// o nome não é válido
				...
				resposta = true;
				String[] partes = nome.split(" "); // trecho para forçar letras minúsculas para maiúsculas, criação do
													// array partes para separar as palavras passadas com espaço, todas
													// as palavras são verificadas por um loop for, depois juntamos as
													// palavras com o append.
				StringBuilder sb = new StringBuilder(); // stringBuilder serve para manipular as palavras, já que String
														// normal não te permite fazer isso

				for (int i = 0; i < partes.length; i++) {
					String word = partes[i];
					word = word.substring(0, 1).toUpperCase() + word.substring(1);
					sb.append(" ").append(word);
					nome = sb.toString().replaceFirst(" ", ""); // replaceFirst é um método pra substituir a primeira
																// letra depois de passado um espaço
				}
		...
		do {
			...
			try { // testa as condicionais, casso ocorra a entrada de uma exceção, ie: passar
					// número onde é esperado string, ao invés do programa encerra, ele trata a
					// exceção no catch.
		...
		System.out.println("\n|Recapitulando... Seu nome é: " + usuario.getNome() + ", você tem " + usuario.getIdade()
				+ " anos e sua cidade é " + usuario.getCidade() + "."); // usando get para receber variaveis do usuario
		...
		if (espacoSuficiente == false && condicaoFinanceira == false && tempoDisponivel == false) {
			...
			System.out.println("\n========================================================");
			System.out.println(
					"\n|{Criamos Esse Programa para Servir de Ponte Justamente pra Conseguir\nTe Direcionar a ONG's que Conseguem Ajudar na Causa}|");
			System.out.println("\n========================================================");
		}
